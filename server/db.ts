...
import { appendAuditEntry, getAuditEntries, verifyAuditChain } from "./auditLedger.ts";
import type {
  CivicIssue,
  IntegrityReport,
  User,
...
export let USERS: User[] = [
  ...
  {
    id: "usr-auditor-01",
    name: "Independent Civic Auditor",
    email: "auditor@civic.gov",
    role: "auditor",
    tenantId: "municipality-sf",
    departmentName: "Office of Independent Oversight",
    badgeNumber: "AUD-001",
    reputationScore: 100,
...
export function createComplaintQueryHash(title: string, description: string): string {
  return generateSHA256Hash(normalizeComplaintText(`${title} ${description}`));
}
...
function persistToFirebase(collection: string, id: string, data: any) {
  if (firebaseDb) {
    if (data === null) {
...
export async function seedDefaultAdmin() {
  ...
  try {
    const adminRef = firebaseDb.ref('users/usr-admin-01');
    const snapshot = await adminRef.once('value');
    
    // If the admin user doesn't exist or is missing the hashed password for 'Scram440', seed it
    const adminData = snapshot.val();
    if (!adminData || !adminData.email || adminData.email !== 'prapti.j') {
      const hashedPassword = bcrypt.hashSync('Scram440', 10);
      ...
      console.log('Default admin (prapti.j) seeded successfully in Firebase.');
    }
  } catch (error) {
...
export const db = {
  ...
  appendAudit: (input: Parameters<typeof appendAuditEntry>[0]) => appendAuditEntry(input),

  getUserById: (id: string) => USERS.find((user) => user.id === id),

  requestApproval: (resourceId: string, action: string, requestedByUserId: string) => {
    const approval = {
    ...
    PENDING_APPROVALS.set(approval.id, approval);
    appendAuditEntry({
      actorUserId: requestedByUserId,
      action: "approval.requested",
      resourceType: "complaint",
      resourceId,
      details: { approvalId: approval.id, requestedAction: action },
    });
    return approval;
  },

  approveRequest: (approvalId: string, approverUserId: string) => {
    const approval = PENDING_APPROVALS.get(approvalId);
    if (!approval || approval.status !== "pending") return null;
    if (approval.requestedByUserId === approverUserId) return "same-actor" as const;
  ...
  adjustUserReputation: (userId: string, amount: number) => {
    const user = USERS.find((candidate) => candidate.id === userId);
    if (!user) return null;
    user.reputationScore = Math.max(0, Math.min(100, (user.reputationScore ?? 100) + amount));
    return user;
  },
  // Issues
  getIssues: (filters?: {
    ...
    if (filters?.tenantId) {
      result = result.filter((i) => !i.tenantId || i.tenantId === filters.tenantId);
    }
    if (filters?.search) {
  ...
  getIssueById: (id: string, tenantId?: string) => {
    return ISSUES.find((i) => i.id === id && (!tenantId || !i.tenantId || i.tenantId === tenantId));
  },

  getIssueByClientId: (clientId: string, tenantId?: string) => ISSUES.find((issue) => issue.clientId === clientId && (!tenantId || issue.tenantId === tenantId)),

  findRecentDuplicate: (userId: string, queryHash: string, windowMs = 24 * 60 * 60 * 1000) => {
    const cutoff = Date.now() - windowMs;
    return ISSUES.find((issue) =>
      issue.userId === userId
      && issue.queryHash === queryHash
      && new Date(issue.createdAt).getTime() >= cutoff
    );
  },

  getSystemicIssueGroups: (windowMs = 7 * 24 * 60 * 60 * 1000, tenantId?: string) => {
    const cutoff = Date.now() - windowMs;
    type IssueGroup = { category: string; subcategory: string; location: string; count: number; issueIds: string[]; userIds: string[] };
    const groups = new Map<string, IssueGroup>();
    ISSUES.filter((issue) => new Date(issue.createdAt).getTime() >= cutoff && (!tenantId || !issue.tenantId || issue.tenantId === tenantId)).forEach((issue) => {
      const key = `${issue.category}|${issue.subcategory}|${issue.address}`;
      const current: IssueGroup = groups.get(key) || {
      ...
      if (!current.userIds.includes(issue.userId)) current.userIds.push(issue.userId);
  ...
  createIssue: (newIssue: Partial<CivicIssue>): CivicIssue => {
    ...
    const issue: any = {
      ...
      queryHash: newIssue.queryHash,
      deviceTag: newIssue.deviceTag,
      clientId: newIssue.clientId,
      moderationStatus: newIssue.moderationStatus || "clear",
      tenantId: newIssue.tenantId || "municipality-sf",
      sourceLanguage: newIssue.sourceLanguage,
      normalizedDescription: newIssue.normalizedDescription,
      estimatedRepairCost: newIssue.estimatedRepairCost,
      wardTag: newIssue.wardTag,
    ...
    appendAuditEntry({
      actorUserId: newIssue.userId || "system",
      action: "complaint.created",
      resourceType: "complaint",
      resourceId: id,
      details: { status: issue.status, category: issue.category },
    });
    return issue;
  },

  updateIssue: (id: string, updates: Partial<CivicIssue>, actorUserId = "system"): CivicIssue | null => {
    ...
    appendAuditEntry({
      ...
      resourceType: "complaint",
      resourceId: id,
      details: { changedFields: Object.keys(updates).filter((key) => key !== "updatedByUserId") },
    });
    return updated;
  },

  deleteIssue: (id: string, actorUserId = "system", tenantId?: string): CivicIssue | null => {
    const idx = ISSUES.findIndex((issue) => issue.id === id && (!tenantId || !issue.tenantId || issue.tenantId === tenantId));
    if (idx === -1) return null;
  ...
  markIssueModeration: (id: string, status: "spam" | "fake" | "clear", actorUserId = "system") => {
    const issue = ISSUES.find((candidate) => candidate.id === id);
    if (!issue) return null;
    const previousStatus = issue.moderationStatus;
    const delta = status === "clear" ? 0 : previousStatus === status ? 0 : -15;
    const updated = db.updateIssue(id, { moderationStatus: status }, actorUserId);
    if (delta) db.adjustUserReputation(issue.userId, delta);
    appendAuditEntry({
      actorUserId,
      action: `complaint.moderation.${status}`,
      resourceType: "complaint",
      resourceId: id,
      details: { previousStatus, reputationDelta: delta },
    });
    return updated;
  },
  ...
  upvoteIssue: (id: string, userId: string): CivicIssue | null => {
    ...
    if (!issue.upvotedUserIds.includes(userId)) {
      ...
      appendAuditEntry({
        ...
        resourceType: "complaint",
        resourceId: id,
        details: { reportCount: issue.reportCount },
      });
    }
    return issue;
  },
