...
export default function InteractiveMap({
  ...
  useEffect(() => {
    ...
    // Dark-themed, crisp OpenStreetMap CartoDB Dark Matter tiles
      ...
      const pinIcon = L.divIcon({
        ...
        html: `
          <div class="relative flex items-center justify-center">
            <div class="w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center text-white font-bold shadow-xl border-2 border-white animate-bounce">
            ...
            <span class="absolute -bottom-6 bg-slate-900 text-indigo-300 text-[10px] font-semibold px-2 py-0.5 rounded shadow border border-indigo-500 whitespace-nowrap">
              Report Location
            </span>
          </div>
        `,
    ...
    issues.forEach((issue) => {
      ...
      const markerHtml = `
        ...
        isSelected ? "border-cyan-400 ring-4 ring-cyan-400/40 scale-125" : "border-white/90"
          ...
          <div class="hidden group-hover:block absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[11px] font-medium px-2 py-0.5 rounded shadow border border-slate-700 whitespace-nowrap z-50 pointer-events-none">
      ...
      // Interactive Popup
      const popupHtml = `
        <div class="w-64 p-1 text-slate-200">
          ${
            issue.initialImageUrl
              ? `<img src="${issue.initialImageUrl}" class="w-full h-24 object-cover rounded-lg mb-2 border border-slate-700" alt="${issue.title}" />`
              : ""
          }
          <div class="flex items-center justify-between gap-1 mb-1">
            ...
            }">${issue.severity} (${issue.priorityScore} Pts)</span>
            <span class="text-[10px] capitalize text-slate-400 font-mono">${issue.status.replace("_", " ")}</span>
          </div>
          ...
          <div class="text-[10px] text-slate-400 flex items-center justify-between border-t border-slate-800 pt-1.5">
  ...
  return (
    <div className={`relative overflow-hidden border border-slate-800 shadow-2xl ${className}`}>
